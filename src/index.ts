...
if (!token) {
  throw new Error(
    "Переменная BOT_TOKEN не найдена. Скопируйте .env.example в .env и вставьте токен Telegram-бота."
  );
}
...
const healthcheckServer = createServer((request, response) => {
  if (request.url === "/") {
...
const startHealthcheckServer = (): Promise<void> => {
  ...
  return new Promise((resolve, reject) => {
    ...
    healthcheckServer.once("error", handleError);
    healthcheckServer.listen(HEALTHCHECK_PORT, () => {
      healthcheckServer.off("error", handleError);
      console.log(`Healthcheck server is running on port ${HEALTHCHECK_PORT}`);
      resolve();
    });
  });
};
...
const launchBot = async () => {
  await Promise.all([startHealthcheckServer(), bot.launch()]);
...
// Корректно останавливаем polling и healthcheck-сервер при завершении процесса.
process.once("SIGINT", () => {
  bot.stop("SIGINT");
